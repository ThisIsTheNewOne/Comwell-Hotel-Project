...
const BookingOverview: React.FC<Props> = (props:Props) => {
  const { id, setDrawerComponent, nextPage, postBooking,  isCreditCardValid } = props;
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 w-full transition-all duration-[400ms] z-[1]">
       <ContinueContainer id={id} setDrawerComponent={setDrawerComponent} nextPage="payment" postBooking={postBooking} isCreditCardValid={isCreditCardValid} />
      </div>
    </>
  );
};
